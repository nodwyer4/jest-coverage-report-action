...
var require_fs = __commonJS({
  "node_modules/fs-extra/lib/fs/index.js"(exports2) {
    ...
    exports2.exists = function(filename2, callback) {
      if (typeof callback === "function") {
        return fs2.exists(filename2, callback);
      }
      return new Promise((resolve) => {
        return fs2.exists(filename2, resolve);
      });
    };
...
var coverageAnnotations = "Coverage report annotations";
...
var errors = {
  unexpectedError: "An unexpected error occurred. For more details, [check console]({{ consoleLink }})",
...
var loadingCoverageFromFile = "Loading code coverage from file: {{ coverageFile }}";
var strings_default = {
  icons,
  stages,
  failed,
  ...
  testResults.forEach(({ assertionResults, name: testResultFilename }) => {
    ...
    annotations.push(...assertionResults.filter(({ status: status2 }) => status2 === "failed").map(({ location, ancestorTitles, title, failureMessages }) => {
      ...
      return {
        ...
        start_line: (_a = location == null ? void 0 : location.line) != null ? _a : 0,
        end_line: (_b = location == null ? void 0 : location.line) != null ? _b : 0,
        title: ancestorTitles == null ? void 0 : ancestorTitles.concat(title).join(" > "),
        message: (0, import_strip_ansi.default)((_c = failureMessages == null ? void 0 : failureMessages.join("\n\n")) != null ? _c : "")
      };
    }));
  });
  return annotations;
...
var formatCoverageAnnotations = (annotations) => {
  ...
  return {
    ...
    status: "completed",
    head_sha: (_b = (_a = import_github.context.payload.pull_request) == null ? void 0 : _a.head.sha) != null ? _b : import_github.context.sha,
    conclusion: "success",
...
var hashOptions = (options) => {
  const optionsToHash = (0, import_pick.default)(options, OPTION_NAMES_TO_HASH);
  return hashString(JSON.stringify(optionsToHash));
};
var getReportTag = (options) => {
...
async function fetchPreviousReport(octokit, repo, pr, options) {
  const commentList = await octokit.paginate("GET /repos/:owner/:repo/issues/:issue_number/comments", {
    ...repo,
    issue_number: pr.number
  });
  const previousReport = commentList.find((comment) => comment.body.startsWith(getReportTag(options)));
  return !previousReport ? null : previousReport;
}

// src/report/generatePRReport.ts
var generatePRReport = async (report, options, repo, pr, octokit) => {
...
var findCommonPath = (filepaths) => {
  ...
  if (filepaths.length) {
    ...
    if (commonRoot.length && commonRoot[commonRoot.length - 1] !== "/") {
      commonRoot = (0, import_path3.dirname)(commonRoot) + "/";
    }
  }
  return commonRoot;
};
...
var getSummary = (map, totalCounter, coveredCounter, title) => {
  ...
  if (map) {
    total = Object.values(map).reduce((acc, currValue) => acc + totalCounter(currValue), 0);
    covered = Object.values(map).reduce((acc, currValue) => acc + coveredCounter(currValue), 0);
  }
  return {
    title,
    total,
...
var shrinkLongPath = (filename2) => {
  const base = (0, import_path4.basename)(filename2);
  if (filename2.length >= LONG_PATH_LENGTH && base !== filename2) {
    return `<div title="${filename2}">\`...\` / ${base}</div>`;
  }
  return filename2;
};
...
var ActionError = class extends Error {
  constructor(reason, details) {
    super(i18n(`errors.${reason}`, details));
  }
  toString() {
    return this.message;
  }
...
var getNumberWidth = (index) => Math.floor(Math.log10(index));
var formatErrors = (errors2) => {
  ...
  return i18n("errors.multiple") + i18n("\n```\n{{ errors }}\n```\n", {
    errors: errors2.map((error, index) => ` ${String(1 + index).padEnd(1 + getNumberWidth(errors2.length), " ")} | ${error.toString()}`).join("\n")
  });
};
...
// src/format/getFailureDetails.ts
var import_strip_ansi2 = __toModule(require_strip_ansi());
var getFailureDetails = ({ testResults }) => {
  if (!testResults || !testResults.some(({ message, status: status2 }) => message.length > 0 && status2 !== "passed")) {
    return "";
  }
  const wrapCode = (code) => "```\n" + code + "\n```";
  const codeBlocks = testResults.filter(({ status: status2 }) => status2 !== "passed").map(({ message }) => {
...
var createReport = (dataCollector, options) => {
  const { workingDirectory, customTitle } = options;
  if (workingDirectory !== void 0) {
    console.log(workingDirectory);
    import_fs.default.readdir(workingDirectory, (err, files) => {
      files.forEach((file) => {
        console.log(file);
      });
    });
...
(function(FailReason2) {
  ...
  FailReason2["REPORT_NOT_FOUND"] = "reportNotFound";
  FailReason2["READING_COVERAGE_FILE_FAILED"] = "readingCoverageFileFailed";
  FailReason2["FAILED_GETTING_COVERAGE"] = "failedGettingCoverage";
...
// src/stages/collectCoverage.ts
var collectCoverage = async (dataCollector, workingDirectory, coverageFile) => {
  const pathToCoverageFile = joinPaths(workingDirectory, REPORT_PATH);
  try {
    if (coverageFile) {
      try {
        dataCollector.info(i18n("loadingCoverageFromFile", { coverageFile }));
        const contents = await (0, import_fs_extra.readFile)(coverageFile);
    ...
    const outBuff = await (0, import_fs_extra.readFile)(pathToCoverageFile);
...
var import_fs_extra3 = __toModule(require_lib2());
var packageScriptRegex = /^(?:(?:npm|yarn|pnpm)\s+(?:run\s+)?([\w:-]+))/;
var isOldScript = async (command, workingDirectory) => {
  ...
  if (matchResult) {
    ...
    try {
      const packageJson = JSON.parse((await (0, import_fs_extra3.readFile)((0, import_path6.join)(...[workingDirectory, "package.json"].filter(Boolean)))).toString());
...
var runTest = async (testCommand, workingDirectory) => {
  await (0, import_exec2.exec)(await getTestCommand(testCommand, "report.json", workingDirectory), [], {
    cwd: workingDirectory
  });
};
...
var getCoverage = async (dataCollector, options, coverageFilePath) => {
  await runStage("install", dataCollector, async (skip) => {
    if (coverageFilePath || !shouldInstallDeps(options.skipStep)) {
      skip();
    }
    await installDependencies(options.packageManager, options.workingDirectory);
  });
  await runStage("runTest", dataCollector, async (skip) => {
    if (coverageFilePath || !shouldRunTestScript(options.skipStep)) {
      skip();
    }
    await runTest(options.testScript, options.workingDirectory);
  });
  const [isCoverageCollected, rawCoverage] = await runStage("collectCoverage", dataCollector, () => collectCoverage(dataCollector, options.workingDirectory, coverageFilePath));
...
// src/run.ts
var run = async (dataCollector = createDataCollector()) => {
  ...
  const [isHeadCoverageGenerated, headCoverage] = await runStage("headCoverage", dataCollector, async () => {
    return await getCoverage(dataCollector, options, options.coverageFile);
  });
  if (headCoverage) {
    dataCollector.add(headCoverage);
  }
  let isSwitched = true;
  if (!options.baseCoverageFile && shouldRunTestScript(options.skipStep)) {
    console.log("switching to base");
    [isSwitched] = await runStage("switchToBase", dataCollector, async (skip) => {
      var _a;
      const baseBranch = (_a = import_github6.context.payload.pull_request) == null ? void 0 : _a.base.ref;
  ...
  const [, baseCoverage2] = await runStage("baseCoverage", dataCollector, async (skip) => {
    ...
    return await getCoverage(ignoreCollector, options, options.baseCoverageFile);
  });
  ...
  await runStage("publishReport", dataCollector, async (skip) => {
    ...
    if (isInPR) {
      await generatePRReport(summaryReport.text, options, import_github6.context.repo, import_github6.context.payload.pull_request, octokit);
    } else {
      await generateCommitReport(summaryReport.text, import_github6.context.repo, octokit);
    }
  });
  ...
  await runStage("coverageAnnotations", dataCollector, async (skip) => {
    ...
    const coverageAnnotations2 = createCoverageAnnotations(headCoverage);
    if (coverageAnnotations2.length === 0) {
      skip();
    }
    const octokit = (0, import_github6.getOctokit)(options.token);
    await octokit.checks.create(formatCoverageAnnotations(coverageAnnotations2));
  });
